...
export function TafsirStudyModal({ open, onOpenChange, surahNumber, surahName, ayahNumber }: Props) {
  ...
  return <Dialog.Root open={open} onOpenChange={onOpenChange}><Dialog.Portal>
    ...
    <Dialog.Content className="fixed left-1/2 top-1/2 z-[71] flex h-[92vh] w-[calc(100%-1.5rem)] max-w-[1400px] -translate-x-1/2 -translate-y-1/2 flex-col overflow-hidden rounded-2xl bg-surface text-ink shadow-2xl outline-none sm:h-[90vh]">
      ...
      <div className="flex shrink-0 items-center gap-3 border-b border-line px-5 py-5 sm:px-12 sm:py-7"><Selector>{surahName}<ChevronDown /></Selector><Selector>{ayahNumber}<ChevronDown /></Selector><button disabled className="control-button opacity-40"><ChevronLeft /></button><button disabled className="control-button opacity-40"><ChevronRight /></button><Dialog.Close className="ml-auto rounded-lg p-2 hover:bg-surface-3"><X className="h-6 w-6" /></Dialog.Close></div>
      <div className="flex-1 overflow-y-auto px-5 pb-10 sm:px-12">
        {loading ? <div className="flex min-h-80 items-center justify-center gap-3 text-ink-muted"><Loader2 className="animate-spin" />Loading Tafsir…</div> : <>
          <section className="py-7 sm:py-9"><div className="flex items-center gap-4 text-ink-faint"><span className="text-xl sm:text-2xl">{surahNumber}:{ayahNumber}</span><Play /><Bookmark /><span className="ml-auto flex gap-5"><Copy /><Share2 /><Pencil /><MoreHorizontal /></span></div><p dir="rtl" translate="no" className="mt-8 text-right font-arabic text-4xl leading-[2] text-ink sm:text-5xl">{verse?.textUthmani}</p><div className="mt-8 grid gap-7 lg:grid-cols-2">{verse?.translations?.map((item) => { const rtl = /^(ur|fa|ar|ps|sd)-/.test(item.translatorSlug) || item.translatorSlug.includes('bayan-ul-quran'); return <div key={item.translatorId} dir={rtl ? 'rtl' : 'ltr'} className={rtl ? 'text-right' : ''}><p className="text-xl leading-9 sm:text-2xl">{item.text}</p><p className="mt-2 text-sm capitalize text-ink-muted">— {item.translatorSlug.replaceAll('-', ' ')}</p></div>; })}</div></section>
          <nav className="flex gap-7 overflow-x-auto border-b border-line text-ink-faint sm:gap-9"><StudyTab icon={<BookOpen />} label="Tafsirs" active /><StudyTab icon={<GraduationCap />} label="Lessons" /><StudyTab icon={<MessageCircle />} label="Reflections" /><StudyTab icon={<BookMarked />} label="Hadith" /><StudyTab icon={<Copy />} label="Related Content" /></nav>
          <div className="flex flex-wrap items-center gap-3 py-5"><button onClick={share} className="control-button"><Share2 /></button><button onClick={() => setFontLarge((v) => !v)} className={`control-button ${fontLarge ? 'text-[var(--accent)]' : ''}`}><Type /></button><select value={language} onChange={(e) => setLanguage(e.target.value)} className="h-10 rounded-lg bg-surface-3 px-4 text-sm outline-none">{languages.map((item) => <option key={item} value={item}>{LANGUAGE_NAMES[item] || item.replace(/\b\w/g, (letter) => letter.toUpperCase())}</option>)}</select>{visibleResources.map((item) => <button key={item.id} onClick={() => setResourceId(item.id)} className={`rounded-full px-4 py-2 text-sm ${resourceId === item.id ? 'bg-[var(--accent)] font-semibold text-white' : 'bg-surface-3 text-ink-3 hover:bg-line'}`}>{item.name}</button>)}</div>
          {groupedRange && <div className="rounded-md bg-[var(--accent)]/10 px-5 py-4 text-sm sm:text-base">You are reading a tafsir for the group of verses {groupedRange}.</div>}
          {contentLoading ? <div className="flex min-h-52 items-center justify-center gap-3 text-ink-muted"><Loader2 className="animate-spin" />Loading {resources.find((item) => item.id === resourceId)?.name}…</div> : tafsir ? <article className={`tafsir-study-content py-7 leading-8 text-ink-2 ${fontLarge ? 'text-xl' : 'text-base sm:text-lg'}`} dangerouslySetInnerHTML={{ __html: cleanHtml(tafsir.text) }} /> : <div className="py-16 text-center text-ink-muted">No Tafsir is available for this verse and source.</div>}
        </>}
      </div>
    </Dialog.Content>

  ...
  return <Dialog.Root open={open} onOpenChange={onOpenChange}><Dialog.Portal>
    ...
    <Dialog.Content className="fixed left-1/2 top-1/2 z-[71] flex h-[92vh] w-[calc(100%-1.5rem)] max-w-[1400px] -translate-x-1/2 -translate-y-1/2 flex-col overflow-hidden rounded-2xl bg-white text-slate-800 shadow-2xl outline-none sm:h-[90vh]">
      ...
      <div className="flex-1 overflow-y-auto px-5 pb-10 sm:px-12">
        {loading ? <div className="flex min-h-80 items-center justify-center gap-3 text-slate-500"><Loader2 className="animate-spin text-[var(--accent)]" />Loading lessons…</div> : error ? <State icon={<AlertCircle />} title="Unable to load lessons" text="The Quran Reflect lesson service is temporarily unavailable. Please try again." /> : <>
          ...
          <nav className="flex gap-7 overflow-x-auto border-b border-slate-200 text-slate-400 sm:gap-9"><StudyTab icon={<BookOpen />} label="Tafsirs" /><StudyTab icon={<GraduationCap />} label="Lessons" active /><StudyTab icon={<MessageCircle />} label="Reflections" /><StudyTab icon={<BookMarked />} label="Hadith" /><StudyTab icon={<Copy />} label="Related Content" /></nav>
          <div className="flex flex-wrap items-center gap-3 py-5"><button onClick={share} disabled={!lesson} className="control-button" aria-label="Share lesson"><Share2 /></button><button onClick={() => setFontLarge((value) => !value)} className={`control-button ${fontLarge ? 'text-[var(--accent)]' : ''}`} aria-label="Change font size"><Type /></button><select value={languageId} onChange={(event) => setLanguageId(Number(event.target.value))} className="h-10 rounded-lg bg-slate-100 px-4 text-sm outline-none">{LANGUAGES.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select>{lessons.length > 0 && <div className="ml-auto flex items-center gap-3"><button disabled={index === 0} onClick={() => setIndex((value) => value - 1)} className="nav-button"><ChevronLeft />Previous</button><span className="text-sm text-slate-500">{index + 1} / {lessons.length}</span><button disabled={index >= lessons.length - 1} onClick={() => setIndex((value) => value + 1)} className="nav-button">Next<ChevronRight /></button></div>}</div>
          <div className="rounded-md border border-cyan-200 bg-cyan-50 px-5 py-4 text-sm leading-6 sm:text-base">Lessons are knowledge-based takeaways connected to this ayah and sourced from moderated Quran Reflect content.</div>
          {!lesson ? <State icon={<GraduationCap />} title="No lessons available" text={`No verified ${LANGUAGES.find((item) => item.id === languageId)?.name || ''} lesson is currently linked to this verse.`} /> : <article className={`py-8 ${languageId === 1 || languageId === 5 ? 'text-right' : ''}`} dir={languageId === 1 || languageId === 5 ? 'rtl' : 'ltr'}><div className="flex items-center gap-3"><div className="flex h-11 w-11 items-center justify-center overflow-hidden rounded-full bg-slate-100 font-semibold text-[var(--accent)]">{lesson.author?.avatarUrls?.small ? <img src={lesson.author.avatarUrls.small} alt="" className="h-full w-full object-cover" /> : authorName.charAt(0)}</div><div><h2 className="font-semibold sm:text-lg">{authorName}</h2>{lesson.author?.username && <p className="text-sm text-slate-500">@{lesson.author.username}</p>}</div></div><div className={`mt-7 whitespace-pre-wrap leading-8 text-slate-800 ${fontLarge ? 'text-xl sm:text-2xl' : 'text-base sm:text-xl'}`}>{lesson.body}</div>{lesson.tags?.length ? <div className="mt-7 flex flex-wrap gap-2">{lesson.tags.map((tag) => <span key={tag.id} className="rounded-full bg-slate-100 px-3 py-1 text-sm text-slate-600">#{tag.name}</span>)}</div> : null}</article>}
        </>}
      </div>
    </Dialog.Content>

  ...
  return <Dialog.Root open={open} onOpenChange={onOpenChange}><Dialog.Portal>
    ...
    <Dialog.Content className="fixed left-1/2 top-1/2 z-[71] flex h-[92vh] w-[calc(100%-1.5rem)] max-w-[1400px] -translate-x-1/2 -translate-y-1/2 flex-col overflow-hidden rounded-2xl bg-surface text-ink shadow-2xl outline-none sm:h-[90vh]">
      ...
      <div className="flex-1 overflow-y-auto px-5 pb-10 sm:px-12">
        {loading ? <div className="flex min-h-80 items-center justify-center gap-3 text-ink-muted"><Loader2 className="animate-spin text-[var(--accent)]" />Loading related content…</div> : error ? <State icon={<AlertCircle />} title="Unable to load related content" text="The related-content service is temporarily unavailable. Please try again." /> : <>
          ...
          {!question ? <State icon={<Copy />} title="No related content available" text="No published question or answer is currently linked to this verse." /> : <article dir={language === 'ar' ? 'rtl' : 'ltr'} className={`py-8 ${language === 'ar' ? 'text-right' : ''}`}><div className="flex flex-wrap gap-2">{question.type && <span className="rounded-full bg-[var(--accent)]/10 px-3 py-1 text-xs font-semibold text-[var(--accent)]">{question.type.replaceAll('_', ' ')}</span>}{question.ranges?.map((range) => <span key={range} className="rounded-full bg-surface-3 px-3 py-1 text-xs text-ink-3">{range}</span>)}</div><h2 className={`mt-6 font-semibold leading-9 text-ink ${fontLarge ? 'text-2xl sm:text-3xl' : 'text-xl sm:text-2xl'}`}>{question.body}</h2><div className="mt-7 space-y-6">{question.answers.map((answer) => <section key={answer.id} className="rounded-2xl border border-line p-5 sm:p-7"><div className={`whitespace-pre-wrap leading-8 ${fontLarge ? 'text-xl sm:text-2xl' : 'text-base sm:text-xl'}`}>{answer.body}</div>{answer.answeredBy && <p className="mt-5 text-sm font-medium text-ink-muted">Answered by {answer.answeredBy}</p>}</section>)}</div>{question.references?.length ? <div className="mt-7"><h3 className="font-semibold">References</h3><ul className="mt-2 list-inside list-disc text-sm text-ink-3">{question.references.map((reference) => <li key={reference}>{reference}</li>)}</ul></div> : null}</article>}
        </>}
      </div>
    </Dialog.Content>

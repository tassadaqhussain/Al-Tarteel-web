  ...
  return <Dialog.Root open={open} onOpenChange={onOpenChange}>
    <Dialog.Portal>
      ...
      <Dialog.Content className="fixed left-1/2 top-1/2 z-[71] flex h-[92vh] w-[calc(100%-1.5rem)] max-w-[1400px] -translate-x-1/2 -translate-y-1/2 flex-col overflow-hidden rounded-2xl bg-white text-slate-800 shadow-2xl outline-none sm:h-[90vh]">
        ...
        <div className="flex shrink-0 items-center gap-3 border-b border-slate-200 px-5 py-5 sm:px-12 sm:py-7">
          ...
          <Dialog.Close className="ml-auto rounded-lg p-2 hover:bg-slate-100" aria-label="Close"><X className="h-6 w-6" /></Dialog.Close>
        </div>

        <div className="flex-1 overflow-y-auto px-5 pb-10 sm:px-12">
          {loading && <div className="flex min-h-80 items-center justify-center gap-3 text-slate-500"><Loader2 className="h-6 w-6 animate-spin text-[var(--accent)]" />Loading study content…</div>}
          ...
          {!loading && !error && <>
            <section className="py-7 sm:py-9">
              <div className="flex items-center gap-4 text-slate-400"><span className="text-xl sm:text-2xl">{surahNumber}:{ayahNumber}</span><Play className="h-6 w-6" /><Bookmark className="h-6 w-6" /><span className="ml-auto flex gap-5"><Copy /><Share2 /><Pencil /><MoreHorizontal /></span></div>
              <p dir="rtl" lang="ar" translate="no" className="mt-8 text-right font-arabic text-4xl leading-[2] text-slate-900 sm:text-5xl">{verse?.textUthmani}</p>
              <div className="mt-8 grid gap-7 lg:grid-cols-2">{verse?.translations?.map((item) => { const rtl = /^(ur|fa|ar|ps|sd)-/.test(item.translatorSlug) || item.translatorSlug.includes('bayan-ul-quran'); return <div key={item.translatorId} dir={rtl ? 'rtl' : 'ltr'} className={rtl ? 'text-right' : ''}><p className="text-xl leading-9 sm:text-2xl">{item.text}</p><p className="mt-2 text-sm capitalize text-slate-500">— {item.translatorSlug.replaceAll('-', ' ')}</p></div>; })}</div>
            </section>

            <nav className="flex gap-7 overflow-x-auto border-b border-slate-200 text-slate-400 sm:gap-9">
            ...
            <div className="flex flex-wrap items-center gap-3 py-5">
              ...
              <select value={language} onChange={(event) => setLanguage(event.target.value as 'en' | 'ar')} className="h-10 rounded-lg border-0 bg-slate-100 px-4 text-sm outline-none"><option value="en">English</option><option value="ar">العربية</option></select>
            ...
            {!hadiths.length ? <State icon={<BookMarked />} title="No Hadith available" text="No verified Hadith reference is linked to this verse." /> : activeHadith && activeText && <article dir={language === 'ar' ? 'rtl' : 'ltr'} className={`py-8 ${language === 'ar' ? 'text-right font-arabic' : ''}`}>
              ...
              {activeText.chapterTitle && <p className="mt-6 text-base text-slate-600 sm:text-lg">{activeText.chapterTitle}</p>}
              <div className={`hadith-copy mt-5 leading-9 text-slate-800 ${fontLarge ? 'text-xl sm:text-2xl' : 'text-base sm:text-xl'}`} dangerouslySetInnerHTML={{ __html: cleanHadithHtml(activeText.body) }} />
              {activeText.grades?.map((grade) => <span key={grade.grade} className="mt-5 inline-flex rounded-full bg-emerald-50 px-3 py-1 text-sm font-semibold text-emerald-700">{grade.grade}</span>)}
            </article>}
          </>}
        </div>
      </Dialog.Content>

  ...
  const dots = [0, 1, 2, 3, 4].map((i) => (
    <span
      ...
      className={cn(
        ...
        stepPercent >= i * 25 ? 'bg-[var(--accent)]' : 'bg-line',
      )}
      aria-hidden
    />
  ));
    ...
    return (
      <div className="mt-8 space-y-4">
        <div className="rounded-2xl border border-line bg-surface p-6 shadow-sm">
          ...
          <h2 className="mt-2 text-2xl font-bold text-ink">{lesson.name}</h2>
          <p className="font-arabic text-xl text-ink-3" lang="ar" dir="rtl">
            {lesson.nameArabic}
          </p>
          {lessonEstimateLabel(slug) && (
            <p className="mt-2 text-sm text-ink-muted">{lessonEstimateLabel(slug)}</p>
          )}
          <div className="mt-5 space-y-3 text-sm text-ink-2">
            ...
            <p className="rounded-xl bg-surface-2 px-4 py-3 text-ink-3">
          ...
          <button
            type="button"
            onClick={startLesson}
            className="mt-6 inline-flex rounded-full bg-[var(--accent)] px-5 py-2.5 text-sm font-bold text-brand-contrast hover:opacity-90"
          >
            Start Lesson
          </button>
        </div>
  ...
  if (sessionStep >= 4 || (mastery === 'reviewed' && sessionStep === 0)) {
    return (
      <div className="mt-8 space-y-5">
        <div className="rounded-2xl border border-brand/20 bg-brand/[0.06] p-6">
          ...
          <p className="mt-4 rounded-xl bg-surface/80 px-4 py-3 text-sm text-ink-2">
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            <Link
              ...
              className="rounded-full border border-line bg-surface px-4 py-2.5 text-sm font-semibold text-ink hover:border-[var(--accent)]"
            >
              Review Examples
            </Link>
            {nextLesson && (
  ...
  return (
    <div className="mt-8 space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-line bg-surface px-4 py-3">
        <div>
          <p className="text-xs font-semibold text-ink-faint">Progress</p>
          <div className="mt-1 flex items-center gap-1.5" aria-label={`${stepPercent}% complete`}>
            {dots}
            <span className="ms-2 text-sm font-semibold text-ink-2">{Math.round(stepPercent)}%</span>
          </div>
        </div>
        <span className="rounded-full bg-surface-3 px-3 py-1 text-xs font-semibold text-ink-3">
      ...
      {banner && (
        <p
          role="status"
          className="rounded-xl border border-line bg-surface-2 px-4 py-3 text-sm text-ink-2"
        >
          {banner}
        </p>
      )}
        ...
        >
          ...
          <ul className="mt-4 space-y-2">
            {lesson.relatedRuleIds.map((id) => {
              ...
              if (!rule) return null;
              return (
                <li key={id} className="flex items-center gap-3 rounded-xl border border-line-subtle px-3 py-2">
            ...
            {lesson.exampleRefs.map((ex) => (
              <li key={`${ex.surah}:${ex.ayah}`}>
                <Link
                  ...
                  className="block rounded-xl border border-line px-4 py-3 text-sm hover:border-[var(--accent)]"
                >
                  <span className="font-semibold text-[var(--accent)]">
                    Surah {ex.surah}, Ayah {ex.ayah}
                  </span>
                  <span className="mt-1 block text-ink-3">{ex.note}</span>
                </Link>
              </li>
            ))}
          </ul>
...
function Section({
  ...
  return (
    <section className="rounded-2xl border border-line bg-surface p-5">
      ...
      <div className="mt-3 text-ink-2 leading-relaxed">{children}</div>
      <button
        type="button"
        onClick={onAction}
        className="mt-5 rounded-full bg-[var(--accent)] px-5 py-2.5 text-sm font-bold text-brand-contrast hover:opacity-90"
      >
        {actionLabel}
      </button>
    </section>

...
export function ExploreTopics() {
  return (
    <section className="w-full px-3 py-3 sm:px-4 sm:py-4 md:px-6">
      <h2 className="mb-3 text-lg font-bold text-slate-800 sm:mb-4 sm:text-xl md:text-2xl">
        Explore Topics
      </h2>
      <div className="-mx-3 flex gap-2 overflow-x-auto px-3 pb-1 sm:-mx-4 sm:gap-3 sm:px-4 md:-mx-6 md:px-6 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        {TOPICS.map((topic) => (
          <Link
            ...
            className="inline-flex shrink-0 items-center gap-1.5 rounded-full bg-slate-100 px-3.5 py-2 text-sm font-medium text-slate-700 transition hover:bg-[var(--accent)]/10 hover:text-[var(--accent)] sm:px-4 sm:py-2.5"
          >
            {topic.label}
